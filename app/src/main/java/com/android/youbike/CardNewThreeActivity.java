...
public class CardNewThreeActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        findViewById(R.id.bt3).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                EditText card = (EditText)findViewById(R.id.iPASSeditText);
                TextView error = (TextView) findViewById(R.id.ipassError);
                error.setText("");
                if(card.getText().toString().trim().length()==0)
                    error.setText("必填");
                else if(!card.getText().toString().trim().matches("^[0-9]{11}") && !card.getText().toString().trim().matches("^[0-9]{16}"))
                    error.setText("格式錯誤");
                else {
                    String r =IpassCardNewPhp.ipassCardNewPhp(card.getText().toString().trim(), account, cookieStr, url);
                    if(r.equals("新增成功")){
                        AlertDialog.Builder builder = new AlertDialog.Builder(CardNewThreeActivity.this);
                        View view1 = LayoutInflater.from(CardNewThreeActivity.this).inflate(R.layout.success_dialog, null);
                        ...
                        AlertDialog.Builder builder = new AlertDialog.Builder(CardNewThreeActivity.this);
                        View view1 = LayoutInflater.from(CardNewThreeActivity.this).inflate(R.layout.error_dialog, null);
